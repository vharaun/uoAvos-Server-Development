...
- uoAvos will always be a work-in-progress project. That means that as long as we are alive and kicking, we will continue to find ways to improve the game engine to allow users to create game servers that could potentially compete with any pixel-art-oriented isometric game on the market! Since our Client and Server are open-source and the proprietors of Ultima Online™ do not own them: you could replace all of the graphics, redo all the data file extensions, and modify the client to read the new extensions as they did the old. Then you could upload your game to Steam as an indie release. 
