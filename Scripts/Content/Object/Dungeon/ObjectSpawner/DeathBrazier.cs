...
{
	public class DeathBrazier : Item
	{
		...
		[Constructable]
		public DeathBrazier() : base(0xE31)
		{
			Name = "Brazier";
			Movable = false;
		...
		public DeathBrazier(Serial serial) : base(serial)
		{
		}
		...
		public Point3D GetSpawnPosition()
		{
			...
			for (var i = 0; i < 10; i++)
			{
				var x = Location.X + (Utility.Random((m_SpawnRange * 2) + 1) - m_SpawnRange);
				var y = Location.Y + (Utility.Random((m_SpawnRange * 2) + 1) - m_SpawnRange);
		...
		public override void OnDoubleClick(Mobile from)
		{
			if (Utility.InRange(from.Location, Location, 2))
			{
				try
				{
					...
					{
						var map = Map;
						var bc = (BaseCreature)Activator.CreateInstance(m_Creatures[Utility.Random(m_Creatures.Length)]);
