...
namespace Server.Items
{
	...
	public abstract class BaseWeapon : Item, IWeapon, IFactionItem, ICraftable, ISlayer, IDurability, IQuality, IEngraved
    {
		...
		public int GetLowerStatReq()
		{
			...
			var v = m_AosWeaponAttributes.LowerStatReq;
		...
		public virtual SkillName GetUsedSkill(Mobile m, bool checkSkillAttrs)
		{
			...
			{
				var swrd = m.Skills[SkillName.Swords].Value;
				var fenc = m.Skills[SkillName.Fencing].Value;
				var mcng = m.Skills[SkillName.Macing].Value;
				double val;
		...
		public virtual bool CheckHit(Mobile attacker, Mobile defender)
		{
			...
			var atkSkill = attacker.Skills[atkWeapon.Skill];
			var defSkill = defender.Skills[defWeapon.Skill];

			var atkValue = atkWeapon.GetAttackSkillValue(attacker, defender);
			var defValue = defWeapon.GetDefendSkillValue(attacker, defender);
		...
		public static bool CheckParry(Mobile defender)
		{
			...
			var parry = defender.Skills[SkillName.Parry].Value;
			var bushidoNonRacial = defender.Skills[SkillName.Bushido].NonRacialValue;
			var bushido = defender.Skills[SkillName.Bushido].Value;

			if (shield != null)
			{
				var chance = (parry - bushidoNonRacial) / 400.0; // As per OSI, no negitive effect from the Racial stuffs, ie, 120 parry and '0' bushido with humans
			...
			else if (!(defender.Weapon is Fists) && !(defender.Weapon is BaseRanged))
			{
				...
				var chance = (parry * bushido) / divisor;
		...
		public virtual int GetPackInstinctBonus(Mobile attacker, Mobile defender)
		{
			...
			foreach (Mobile m in eable)
			{
				if (m != attacker && m is BaseCreature)
				{
					...
					var theirMaster = tc.ControlMaster;
		...
		public virtual void OnHit(Mobile attacker, Mobile defender, double damageBonus)
		{
			...
			{
				var pmAttacker = (PlayerMobile)attacker;
			...
			var ignoreArmor = (a is ArmorIgnore || (move != null && move.IgnoreArmor(attacker)));
			...
			var propertyBonus = (move == null) ? 1.0 : move.GetPropertyBonus(attacker);
			...
			if (Core.AOS)
			{
				var physChance = (int)(AosWeaponAttributes.GetValue(attacker, AosWeaponAttribute.HitPhysicalArea) * propertyBonus);
				var fireChance = (int)(AosWeaponAttributes.GetValue(attacker, AosWeaponAttribute.HitFireArea) * propertyBonus);
				var coldChance = (int)(AosWeaponAttributes.GetValue(attacker, AosWeaponAttribute.HitColdArea) * propertyBonus);
				var poisChance = (int)(AosWeaponAttributes.GetValue(attacker, AosWeaponAttribute.HitPoisonArea) * propertyBonus);
				var nrgyChance = (int)(AosWeaponAttributes.GetValue(attacker, AosWeaponAttribute.HitEnergyArea) * propertyBonus);
				...
				var maChance = (int)(AosWeaponAttributes.GetValue(attacker, AosWeaponAttribute.HitMagicArrow) * propertyBonus);
				var harmChance = (int)(AosWeaponAttributes.GetValue(attacker, AosWeaponAttribute.HitHarm) * propertyBonus);
				var fireballChance = (int)(AosWeaponAttributes.GetValue(attacker, AosWeaponAttribute.HitFireball) * propertyBonus);
				var lightningChance = (int)(AosWeaponAttributes.GetValue(attacker, AosWeaponAttribute.HitLightning) * propertyBonus);
				var dispelChance = (int)(AosWeaponAttributes.GetValue(attacker, AosWeaponAttribute.HitDispel) * propertyBonus);
				...
				var laChance = (int)(AosWeaponAttributes.GetValue(attacker, AosWeaponAttribute.HitLowerAttack) * propertyBonus);
				var ldChance = (int)(AosWeaponAttributes.GetValue(attacker, AosWeaponAttribute.HitLowerDefend) * propertyBonus);
		...
		public virtual double GetAosDamage(Mobile attacker, int bonus, int dice, int sides)
		{
			...
			// Inscription bonus
			var inscribeSkill = attacker.Skills[SkillName.Inscribe].Fixed;
		...
		{
			var atkWeapon = attacker.Weapon as BaseWeapon;
			var atkSlayer = SlayerGroup.GetEntryByName(atkWeapon.Slayer);
			var atkSlayer2 = SlayerGroup.GetEntryByName(atkWeapon.Slayer2);
		...
		public virtual double ScaleDamageAOS(Mobile attacker, double damage, bool checkSkills)
		{
			...
			var anatomyBonus = GetBonus(attacker.Skills[SkillName.Anatomy].Value, 0.500, 100.0, 5.00);
			var tacticsBonus = GetBonus(attacker.Skills[SkillName.Tactics].Value, 0.625, 100.0, 6.25);
			var lumberBonus = GetBonus(attacker.Skills[SkillName.Lumberjacking].Value, 0.200, 100.0, 10.00);
			...
			var totalBonus = strengthBonus + anatomyBonus + tacticsBonus + lumberBonus + ((GetDamageBonus() + damageBonus) / 100.0);
		...
		public virtual double ScaleDamageOld(Mobile attacker, double damage, bool checkSkills)
		{
			 ...
			 */
			var anatomyValue = attacker.Skills[SkillName.Anatomy].Value;
			...
			if (Type == WeaponType.Axe)
			{
				var lumberValue = attacker.Skills[SkillName.Lumberjacking].Value;
	...
	#region Affixed Weapon Attributes
	...
	#region Passive Equip And Unequip
	/// Added Code: AutoToggleWeapon.From(this); (To Line 3180 In PlayerMobile.cs)

	static class AutoToggleWeapon
	{
		public static class AutoToggleWeaponSystem
		{
		...
		{
			if (AutoToggleWeaponSystem.Enabled)
			{
		...
		public static void From(Mobile m)
		{
			...
			if (m.Warmode)
			{
				if ((weapon == null || AllowedToKeep(weapon)) && lastWeapon != null && lastWeapon.IsChildOf(m.Backpack) && lastWeapon.Movable && lastWeapon.Visible && !lastWeapon.Deleted)
				{
					...
					if (AutoToggleWeaponSystem.SendOverheadMessage)
					{
