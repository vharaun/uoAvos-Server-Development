...
namespace Server.Items
{
	public class BallOfSummoning : Item, ITranslocationItem
	{
		...
		private class PetLinkTarget : Target
		{
			...
			protected override void OnTarget(Mobile from, object targeted)
			{
				...
				else if (targeted is BaseCreature creature)
				{
					...
					{
						from.SendLocalizedMessage(1054117, String.Empty, 0x59); // You may only link your own pets to a Crystal Ball of Pet Summoning.
					}
					else if (!creature.IsBonded)
					{
						from.SendLocalizedMessage(1054118, String.Empty, 0x59); // You must bond with your pet before it can be linked to a Crystal Ball of Pet Summoning.
					}
					else
					{
						from.SendLocalizedMessage(1054119, String.Empty, 0x59); // Your pet is now linked to this Crystal Ball of Pet Summoning.
				...
				{
					from.SendLocalizedMessage(1054115, String.Empty, 0x59); // The Crystal Ball of Pet Summoning cannot summon itself.
				}
				else
				{
					from.SendLocalizedMessage(1054116, String.Empty, 0x59); // Only pets can be linked to this Crystal Ball of Pet Summoning.
				}
			}
		}
		...
		public void CastSummonPet(Mobile from)
		{
			...
			{
				from.SendLocalizedMessage(1054124, String.Empty, 0x36); // The Crystal Ball fills with a yellow mist. Why would you summon your pet while riding it?
			}
			else if (pet.Map == Map.Internal && (!pet.IsStabled || (from.Followers + pet.ControlSlots) > from.FollowersMax))
			{
				from.SendLocalizedMessage(1054125, String.Empty, 0x5); // The Crystal Ball fills with a blue mist. Your pet is not responding to the summons.
			}
			else if ((!pet.Controlled || pet.ControlMaster != from) && !from.Stabled.Contains(pet))
			{
				from.SendLocalizedMessage(1054126, String.Empty, 0x8FD); // The Crystal Ball fills with a grey mist. You are not the owner of the pet you are attempting to summon.
			}
			else if (!pet.IsBonded)
			{
				from.SendLocalizedMessage(1054127, String.Empty, 0x22); // The Crystal Ball fills with a red mist. You appear to have let your bond to your pet deteriorate.
			}
			...
			{
				_ = from.Send(new AsciiMessage(Serial, ItemID, MessageType.Regular, 0x22, 3, "", "You cannot summon your pet to this location."));
			}
			else if (Core.ML && from is PlayerMobile && DateTime.UtcNow < ((PlayerMobile)from).LastPetBallTime.AddSeconds(15.0))
			{
				from.SendLocalizedMessage(1080072, String.Empty, 0x22); // You must wait a few seconds before you can summon your pet.
			}
			else
			{
				if (Core.ML)
				{
					_ = new PetSummoningSpell(this, from).Cast();
				}
				else
				{
					SummonPet(from);
		...
		public void SummonPet(Mobile from)
		{
			...
			if (pet.IsStabled)
			{
				...
				pet.IsStabled = false;
				pet.StabledBy = null;
				_ = from.Stabled.Remove(pet);

				if (from is PlayerMobile)
				{
					_ = ((PlayerMobile)from).AutoStabled.Remove(pet);
				}
			}

			pet.MoveToWorld(from.Location, from.Map);

			from.SendLocalizedMessage(1054128, String.Empty, 0x43); // The Crystal Ball fills with a green mist. Your pet has been summoned.
		...
		private class PetSummoningSpell : Spell
		{
			...
			{
				return type is not SpellInterrupt.EquipRequest and not SpellInterrupt.UseRequest;
			}
