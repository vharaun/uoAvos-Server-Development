...
namespace Server.Engines.BulkOrders
{
	...
	public abstract class RewardCalculator
	{
		...
		public RewardGroup[] Groups { get => m_Groups; set => m_Groups = value; }
		...
		{
			for (var i = m_Groups.Length - 1; i >= 1; --i)
			{
	...
	public sealed class SmithRewardCalculator : RewardCalculator
	{
		...
		public override int ComputeGold(int quantity, bool exceptional, BulkMaterialType material, int itemCount, Type type)
		{
			...
			var quanIndex = (quantity == 20 ? 2 : quantity == 15 ? 1 : 0);
			var mtrlIndex = (material >= BulkMaterialType.DullCopper && material <= BulkMaterialType.Valorite) ? 1 + (material - BulkMaterialType.DullCopper) : 0;
	...
	public sealed class TailorRewardCalculator : RewardCalculator
	{
		...
		public override int ComputeGold(int quantity, bool exceptional, BulkMaterialType material, int itemCount, Type type)
		{
			...
			var typeIndex = ((itemCount == 6 ? 3 : itemCount == 5 ? 2 : itemCount == 4 ? 1 : 0) * 2) + (exceptional ? 1 : 0);
			var quanIndex = (quantity == 20 ? 2 : quantity == 15 ? 1 : 0);
			var mtrlIndex = (material == BulkMaterialType.Barbed ? 3 : material == BulkMaterialType.Horned ? 2 : material == BulkMaterialType.Spined ? 1 : 0);
