...
namespace Server.Engines.ConPVP
{
	...
	public class TournamentRegistrar : Banker
	{
		...
		public TournamentController Tournament { get => m_Tournament; set => m_Tournament = value; }
	...
	public class TournamentSignupItem : Item
	{
		...
		[CommandProperty(AccessLevel.GameMaster)]
		public Mobile Registrar { get => m_Registrar; set => m_Registrar = value; }
		...
		public override void OnDoubleClick(Mobile from)
		{
			...
			{
				var tourny = m_Tournament.Tournament;
	...
	public class ConfirmSignupGump : Gump
	{
		...
		public ConfirmSignupGump(Mobile from, Mobile registrar, Tournament tourny, ArrayList players) : base(50, 50)
		{
			...
			if (tourny.PlayersPerParticipant > 1)
			{
				...
				for (var i = players.Count; i < tourny.PlayersPerParticipant; ++i, y += 20)
				{
					if (i == 0)
					{
						AddImage(35, y, 0xD2);
					...
					case TournamentStage.Signup:
						{
							...
							{
								var mob = (Mobile)m_Players[i];

								var entry = (ladder == null ? null : ladder.Find(mob));
							...
							if (m_Registrar != null)
							{
								...
								string timeUntil;
								var minutesUntil = (int)Math.Round(((tourny.SignupStart + tourny.SignupPeriod) - DateTime.UtcNow).TotalMinutes);
							...
							var part = new TournyParticipant(from);
							part.Players.Clear();
	...
	public class TournamentController : Item
	{
		...
		public static bool IsActive
		{
			get
			{
				...
				{
					var controller = (TournamentController)m_Instances[i];
	...
	public class Tournament
	{
		...
		public bool HasParticipant(Mobile mob)
		{
			...
			{
				var part = (TournyParticipant)m_Participants[i];
		...
		public void HandleTie(Arena arena, TournyMatch match, ArrayList remaining)
		{
			...
			{
				var part = (TournyParticipant)match.Participants[j];
			...
			switch (m_TieType)
			{
				...
				case TieType.Random:
					{
						var advanced = (TournyParticipant)remaining[Utility.Random(remaining.Count)];
				...
				case TieType.Highest:
					{
						...
						for (var i = 0; i < remaining.Count; ++i)
						{
							var part = (TournyParticipant)remaining[i];
		...
		public void OnEliminated(DuelPlayer player)
		{
			...
			if (m_TournyType == TournyType.FreeForAll)
			{
				...
				{
					var check = (Participant)part.Context.Participants[i];
				...
				case TournyType.FreeForAll:
					{
						...
						var top = m_Pyramid.Levels[m_Pyramid.Levels.Count - 1] as PyramidLevel;
						...
						var match = top.Matches[0] as TournyMatch;
				...
				case TournyType.Standard:
					{
						...
						{
							var part = (TournyParticipant)match.Participants[i];
						...
						var next = m_Pyramid.Levels[m_Pyramid.Levels.Count - 2] as PyramidLevel;
		...
		public void Slice()
		{
			...
			{
				var until = (m_SignupStart + m_SignupPeriod) - DateTime.UtcNow;

				if (until <= TimeSpan.Zero)
				{
					for (var i = m_Participants.Count - 1; i >= 0; --i)
					{
						var part = (TournyParticipant)m_Participants[i];
						var bad = false;
						...
						{
							var check = (Mobile)part.Players[j];
					...
					if (m_Participants.Count >= 2)
					{
						...
						var level = (PyramidLevel)m_Pyramid.Levels[0];
						...
						{
							var match = (TournyMatch)level.Matches[i];
			...
			else if (m_Stage == TournamentStage.Fighting)
			{
				...
				{
					var winner = (TournyParticipant)m_Undefeated[0];
				...
				{
					var activeLevel = (PyramidLevel)m_Pyramid.Levels[m_Pyramid.Levels.Count - 1];
					var stillGoing = false;

					for (var i = 0; i < activeLevel.Matches.Count; ++i)
					{
						var match = (TournyMatch)activeLevel.Matches[i];
						...
						if (match.Winner == null)
						{
							...
							if (!match.InProgress)
							{
								for (var j = 0; j < m_Arenas.Count; ++j)
								{
									var arena = (Arena)m_Arenas[j];
					...
					if (!stillGoing)
					{
						for (var i = m_Undefeated.Count - 1; i >= 0; --i)
						{
							var part = (TournyParticipant)m_Undefeated[i];
							var bad = false;
	...
	public class TournyPyramid
	{
		...
		{
			var copy = new ArrayList(participants);
				...
				case TournyType.RedVsBlue:
					{
						...
						{
							var players = ((TournyParticipant)copy[i]).Players;
				...
				case TournyType.Standard:
					{
						...
						{
							var lowAdvances = Int32.MaxValue;

							for (var i = 0; i < participants.Count; ++i)
							{
								var p = (TournyParticipant)participants[i];
							...
							var idx = Utility.Random(toAdvance.Count);
	...
	public class TournyMatch
	{
		...
		{
			var first = (TournyParticipant)m_Participants[0];

			var dc = new DuelContext((Mobile)first.Players[0], tourny.Ruleset.Layout, false);
			dc.Ruleset.Options.SetAll(false);
			dc.Ruleset.Options.Or(tourny.Ruleset.Options);

			for (var i = 0; i < m_Participants.Count; ++i)
			{
				var tournyPart = (TournyParticipant)m_Participants[i];
				var duelPart = new Participant(dc, tournyPart.Players.Count) {
					TournyPart = tournyPart
				};

				for (var j = 0; j < tournyPart.Players.Count; ++j)
				{
			...
			if (dc.StartedBeginCountdown)
			{
				...
				for (var i = 0; i < m_Participants.Count; ++i)
				{
					var p = (TournyParticipant)m_Participants[i];

					for (var j = 0; j < p.Players.Count; ++j)
					{
						var mob = (Mobile)p.Players[j];
		...
		public TournyMatch(ArrayList participants)
		{
			...
			for (var i = 0; i < participants.Count; ++i)
			{
				var part = (TournyParticipant)participants[i];
		...
		public TournamentBracketGump(Mobile from, Tournament tourny, TournyBracketGumpType type, ArrayList list, int page, object obj) : base(50, 50)
		{
			...
			switch (type)
			{
				...
				case TournyBracketGumpType.Participant_Info:
					{
						var part = obj as TournyParticipant;
				...
				case TournyBracketGumpType.Round_List:
					{
						...
						for (var i = 0; i < count; ++i, y += 18)
						{
							var level = (PyramidLevel)m_List[index + i];
				...
				case TournyBracketGumpType.Round_Info:
					{
						...
						var level = m_Object as PyramidLevel;
						...
						for (var i = 0; i < count; ++i, y += 18)
						{
							var match = (TournyMatch)m_List[index + i];
				...
				case TournyBracketGumpType.Match_Info:
					{
						...
						var ct = (m_Tournament.TournyType == TournyType.FreeForAll ? 2 : match.Participants.Count);
				...
				case 0:
					{
						switch (index)
						{
							...
							case 3:
								{
									...
									{
										var part = (TournyParticipant)m_Tournament.Participants[i];
							...
							case 5:
								{
									var match = m_Object as TournyMatch;
									...
									{
										var level = (PyramidLevel)m_Tournament.Pyramid.Levels[i];
				...
				case 4:
					{
						...
						var part = m_Object as TournyParticipant;
