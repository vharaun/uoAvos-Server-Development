...
namespace Server.Engines.Events
{
	public class NaughtyOrNice
	{
		...
		public static void Initialize()
		{
			...
			if (DateTime.UtcNow >= HolidaySettings.StartChristmas && DateTime.UtcNow <= HolidaySettings.FinishChristmas)
			{
		...
		private static void EventSink_Speech(SpeechEventArgs e)
		{
			...
			if (Insensitive.Contains(e.Speech, "merry christmas"))
			{
				e.Mobile.Target = new NaughtyOrNiceTarget();

				// e.Mobile.SendLocalizedMessage(1076764);  /* Pick someone to Trick or Treat. */
			}
		}

		private class NaughtyOrNiceTarget : Target
		{
			public NaughtyOrNiceTarget()
			...
			protected override void OnTarget(Mobile from, object targ)
			{
				if (targ != null && CheckMobile(from))
				{
					...
					{
						if (m_Begged.NextNaughtyOrNice > now)
						{
							// from.SendLocalizedMessage(1076767); /* That doesn't appear to have any more candy. */
							return;
						}

						m_Begged.NextNaughtyOrNice = now + TimeSpan.FromMinutes(Utility.RandomMinMax(5, 10));
						...
						if (from.Backpack != null && !from.Backpack.Deleted)
						{
							if (Utility.RandomDouble() > .10)
							{
								...
								{
									from.AddToBackpack(HolidaySettings.RandomChristmasItem);

									from.SendLocalizedMessage(1076777); /* You receive a special treat! */
								}
								else
								{
									from.AddToBackpack(HolidaySettings.RandomChristmasTreat);
