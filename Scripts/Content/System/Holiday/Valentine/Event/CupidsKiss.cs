...
namespace Server.Engines.Events
{
	public class CupidsKiss
	{
		...
		public static void Initialize()
		{
			...
			if (DateTime.UtcNow >= HolidaySettings.StartValentine && DateTime.UtcNow <= HolidaySettings.FinishValentine)
			{
		...
		private static void EventSink_Speech(SpeechEventArgs e)
		{
			...
			if (Insensitive.Contains(e.Speech, "happy valentines day"))
			{
				e.Mobile.Target = new CupidKissTarget();

				// e.Mobile.SendLocalizedMessage(1076764);  /* Pick someone to Trick or Treat. */
			}
		}

		private class CupidKissTarget : Target
		{
			public CupidKissTarget()
			...
			protected override void OnTarget(Mobile from, object targ)
			{
				if (targ != null && CheckMobile(from))
				{
					...
					{
						if (m_Begged.NextCupidsKiss > now)
						{
							// from.SendLocalizedMessage(1076767); /* That doesn't appear to have any more candy. */
							return;
						}

						m_Begged.NextCupidsKiss = now + TimeSpan.FromMinutes(Utility.RandomMinMax(5, 10));
						...
						if (from.Backpack != null && !from.Backpack.Deleted)
						{
							if (Utility.RandomDouble() > .10)
							{
								...
								{
									from.AddToBackpack(HolidaySettings.RandomValentineItem);

									from.SendAsciiMessage(""); /* You receive a special treat! */
								}
								else
								{
									from.AddToBackpack(HolidaySettings.RandomValentineTreat);
