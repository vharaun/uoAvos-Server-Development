...
namespace Server.Mobiles
{
	public class BaseEscortable : BaseCreature
	{
		...
		public static readonly TimeSpan AbandonDelay = ChainQuestSystem.Enabled ? TimeSpan.FromMinutes(1.0) : TimeSpan.FromMinutes(2.0);
		public static readonly TimeSpan DeleteTime = ChainQuestSystem.Enabled ? TimeSpan.FromSeconds(100) : TimeSpan.FromSeconds(30);

		public override bool StaticChainQuester => false;  // Suppress automatic quest registration on creation/deserialization

		private ChainQuest m_ChainQuest;

		protected override List<ChainQuest> ConstructQuestList()
		{
			if (m_ChainQuest == null)
			{
				var reg = Region;
				var list = reg.IsPartOf("Haven Island") ? m_ChainQuestTypesNH : m_ChainQuestTypes;
				...
				for (var i = 0; i < list.Length; ++i)
				{
					...
					else if (ChainQuestSystem.Debug)
					{
		...
		public override void Shout(PlayerMobile pm)
		{
			 ...
			 */
			ChainQuestSystem.Tell(this, pm, Utility.Random(1072301, 3));
		}
		...
		// Classic list
		// Used when: !ChainQuestSystem.Enabled && !Core.ML
		...
		// Used when: ChainQuestSystem.Enabled && !Region.IsPartOf( "Haven Island" )
		private static readonly Type[] m_ChainQuestTypes =
		{
			typeof( EscortToYew ),
		...
		// Used when: ChainQuestSystem.Enabled && Region.IsPartOf( "Haven Island" )
		private static readonly Type[] m_ChainQuestTypesNH =
		{
		...
		public virtual Mobile GetEscorter()
		{
			...
			if (ChainQuestSystem.Enabled || master == null)
			{
				return master;
			}
		...
		public override void Serialize(GenericWriter writer)
		{
			...
			ChainQuestSystem.WriteQuestRef(writer, StaticChainQuester ? null : m_ChainQuest);
		}
		...
		public override void Deserialize(GenericReader reader)
		{
			...
			if (version >= 1)
			{
				...
				if (ChainQuestSystem.Enabled && quest != null && !StaticChainQuester)
				{
					m_ChainQuest = quest;
				}
			}
		...
		public override void AddCustomContextEntries(Mobile from, List<ContextMenuEntry> list)
		{
			if (from.Alive)
			{
				...
				if (!ChainQuestSystem.Enabled && GetDestination() != null)
				{
