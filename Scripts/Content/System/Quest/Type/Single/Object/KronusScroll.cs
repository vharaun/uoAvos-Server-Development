...
namespace Server.Engines.Quests.Items
{
	public class KronusScroll : QuestItem
	{
		...
		public override bool CanDrop(PlayerMobile player)
		{
			...
			/*return !( qs.IsObjectiveInProgress( typeof( FindCallingScrollObjective_DarkTidesQuest ) )
				|| qs.IsObjectiveInProgress( typeof( FindMardothAboutKronusObjective_DarkTidesQuest ) )
				|| qs.IsObjectiveInProgress( typeof( FindWellOfTearsObjective_DarkTidesQuest ) )
				|| qs.IsObjectiveInProgress( typeof( UseCallingScrollObjective_DarkTidesQuest ) ) );*/
			return false;
		}
		...
		public override void OnDoubleClick(Mobile from)
		{
			...
			if (pm != null)
			{
				...
				{
					if (qs.IsObjectiveInProgress(typeof(FindMardothAboutKronusObjective_DarkTidesQuest)))
					{
					...
					else if (qs.IsObjectiveInProgress(typeof(FindWellOfTearsObjective_DarkTidesQuest)))
					{
						pm.SendLocalizedMessage(1060152, "", 0x41); // You must be at the Well of Tears in the city of Necromancers to use this scroll.
					}
					else if (qs.IsObjectiveInProgress(typeof(UseCallingScrollObjective_DarkTidesQuest)))
					{
						if (pm.Map == m_WellOfTearsMap && m_WellOfTearsArea.Contains(pm))
						{
							var obj = qs.FindObjective(typeof(UseCallingScrollObjective_DarkTidesQuest));
		...
		private class CallingTimer : Timer
		{
			...
			protected override void OnTick()
			{
				...
				if (m_Step == 4)
				{
					var baseX = KronusScroll.m_WellOfTearsArea.X;
					var baseY = KronusScroll.m_WellOfTearsArea.Y;
					var width = KronusScroll.m_WellOfTearsArea.Width;
					var height = KronusScroll.m_WellOfTearsArea.Height;
					var map = KronusScroll.m_WellOfTearsMap;
