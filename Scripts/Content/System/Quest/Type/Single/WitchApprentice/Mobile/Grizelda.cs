...
namespace Server.Engines.Quests.Mobiles
{
	public class Grizelda : BaseQuester
	{
		...
		public override void OnTalk(PlayerMobile player, bool contextMenu)
		{
			...
			{
				if (qs.IsObjectiveInProgress(typeof(FindApprenticeObjective_WitchApprenticeQuest)))
				{
					PlaySound(0x259);
					PlaySound(0x206);
					qs.AddConversation(new HagDuringCorpseSearchConversation_WitchApprenticeQuest());
				}
				else
				{
					var obj = qs.FindObjective(typeof(FindGrizeldaAboutMurderObjective_WitchApprenticeQuest));
					...
					else if (qs.IsObjectiveInProgress(typeof(KillImpsObjective_WitchApprenticeQuest))
						|| qs.IsObjectiveInProgress(typeof(FindZeefzorpulObjective_WitchApprenticeQuest)))
					{
						PlaySound(0x259);
						PlaySound(0x206);
						qs.AddConversation(new HagDuringImpSearchConversation_WitchApprenticeQuest());
					}
					else
					{
						obj = qs.FindObjective(typeof(ReturnRecipeObjective_WitchApprenticeQuest));
						...
						else if (qs.IsObjectiveInProgress(typeof(FindIngredientObjective_WitchApprenticeQuest)))
						{
							PlaySound(0x259);
							PlaySound(0x206);
							qs.AddConversation(new HagDuringIngredientsConversation_WitchApprenticeQuest());
						}
						else
						{
							obj = qs.FindObjective(typeof(ReturnIngredientsObjective_WitchApprenticeQuest));
			...
			else
			{
				...
				if (qs != null)
				{
					newQuest.AddConversation(new DontOfferConversation_WitchApprenticeQuest());
				}
				...
				else if (inRestartPeriod)
				{
					PlaySound(0x259);
					PlaySound(0x206);
					newQuest.AddConversation(new RecentlyFinishedConversation_WitchApprenticeQuest());
				}
			}
		}
