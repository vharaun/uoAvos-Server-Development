...
namespace Server.Engines.Quests.Mobiles
{
	public class AnsellaGryen : BaseQuester
	{
		...
		public override void OnTalk(PlayerMobile player, bool contextMenu)
		{
			...
			{
				if (qs.IsObjectiveInProgress(typeof(FirstKillObjective_TerribleHatchlingsQuest)))
				{
					qs.AddConversation(new DirectionConversation_TerribleHatchlingsQuest());
				}
				else if (qs.IsObjectiveInProgress(typeof(SecondKillObjective_TerribleHatchlingsQuest))
					|| qs.IsObjectiveInProgress(typeof(ThirdKillObjective_TerribleHatchlingsQuest)))
				{
					qs.AddConversation(new TakeCareConversation_TerribleHatchlingsQuest());
				}
				else
				{
					var obj = qs.FindObjective(typeof(ReturnObjective_TerribleHatchlingsQuest));
