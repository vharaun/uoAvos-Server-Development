	...
	public partial class Weather
	{
		...
		public static void AddDynamicWeather(int temperature, int chanceOfPercipitation, int chanceOfExtremeTemperature, int moveSpeed, int width, int height, Rectangle2D bounds)
		{
			for (var i = 0; i < m_Facets.Length; ++i)
			{
				...
				var w = new Weather(m_Facets[i], new Rectangle2D[] { area }, temperature, chanceOfPercipitation, chanceOfExtremeTemperature, TimeSpan.FromSeconds(30.0)) {
					m_Bounds = bounds,
		...
		public virtual void Reposition()
		{
			...
			var width = m_Area[0].Width;
			var height = m_Area[0].Height;
		...
		public virtual void MoveForward()
		{
			...
			{
				var xOffset = (m_MoveSpeed * m_MoveAngleX) / 100;
				var yOffset = (m_MoveSpeed * m_MoveAngleY) / 100;

				var oldArea = m_Area[0];
		...
		public virtual void OnTick()
		{
			...
			if (m_Active)
			{
				...
				for (var i = 0; i < states.Count; ++i)
				{
					...
					var contains = (m_Area.Length == 0);

					for (var j = 0; !contains && j < m_Area.Length; ++j)
					{
