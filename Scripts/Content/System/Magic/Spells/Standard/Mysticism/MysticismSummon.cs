...
{
	public abstract class MysticismSummon<T> : MysticismSpell where T : BaseCreature
	{
		...
		public MysticismSummon(Mobile caster, Item scroll, MysticismSpellName id)
			: base(caster, scroll, id)
		{
		}

		public MysticismSummon(Mobile caster, Item scroll, SpellInfo info)
		...
		public virtual int GetCount()
		{
			if (Count <= 0)
			{
				return Math.Min(1, Caster.FollowersMax - Caster.Followers);
			}
		...
		{
			return TimeSpan.FromMinutes(Caster.Skills.Mysticism.Value / 24.0 + 2.0);
		}
		...
		{
			private readonly MysticismSummon<T> m_Owner;

			public InternalTarget(MysticismSummon<T> owner)
