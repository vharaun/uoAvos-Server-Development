...
namespace Server.Spells.Mysticism
{
	public class NetherCycloneSpell : MysticismSpell
	{
		...
		public NetherCycloneSpell(Mobile caster, Item scroll)
			: base(caster, scroll, MysticismSpellName.NetherCyclone)
		{
		}
		...
		public void Target(IPoint3D p)
		{
			...
			else if (CheckSequence())
			{
				...
				foreach (var m in Caster.Map.GetMobilesInRange(new Point3D(p), 2))
				{
					if (m == Caster)
					{
						continue;
		...
		{
			private readonly NetherCycloneSpell m_Owner;
