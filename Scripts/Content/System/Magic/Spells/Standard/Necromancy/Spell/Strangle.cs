...
{
	public class StrangleSpell : NecromancySpell
	{
		public override TimeSpan CastDelayBase => TimeSpan.FromSeconds(2.0);

		public StrangleSpell(Mobile caster, Item scroll) 
			: base(caster, scroll, NecromancySpellName.Strangle)
		{
		}
		...
		public void Target(Mobile m)
		{
			...
			var spiritlevel = Caster.Skills[SkillName.SpiritSpeak].Value / 10;
			if (spiritlevel < 4)
			{
				spiritlevel = 4;
			}
			...
			var args = String.Format("{0}\t{1}", d_MinDamage, d_MaxDamage);
		...
		private class InternalTimer : Timer
		{
			...
			public InternalTimer(Mobile target, Mobile from) : base(TimeSpan.FromSeconds(0.1), TimeSpan.FromSeconds(0.1))
			{
				...
				var spiritLevel = from.Skills[SkillName.SpiritSpeak].Value / 10;
