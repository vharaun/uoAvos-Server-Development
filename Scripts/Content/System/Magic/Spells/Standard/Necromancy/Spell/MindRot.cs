...
{
	public class MindRotSpell : NecromancySpell
	{
		public override TimeSpan CastDelayBase => TimeSpan.FromSeconds(1.5);

		public MindRotSpell(Mobile caster, Item scroll) 
			: base(caster, scroll, NecromancySpellName.MindRot)
		{
		}
		...
		public void Target(Mobile m)
		{
			...
			else if (CheckHSequence(m))
			{
				...
				var duration = TimeSpan.FromSeconds((((GetDamageSkill(Caster) - GetResistSkill(m)) / 5.0) + 20.0) * (m.Player ? 1.0 : 2.0));
		...
		public static void ClearMindRotScalar(Mobile m)
		{
			...
			var tmpB = (MRBucket)m_Table[m];
			var tmpT = tmpB.m_MRExpireTimer;
			tmpT.Stop();
			m_Table.Remove(m);
		...
		public static void SetMindRotScalar(Mobile caster, Mobile target, double scalar, TimeSpan duration)
		{
			if (!m_Table.ContainsKey(target))
			{
				...
				var tmpB = (MRBucket)m_Table[target];
				var tmpT = tmpB.m_MRExpireTimer;
				tmpT.Start();
