...
{
	public class EvasionSpell : BushidoSpell
	{
		private static readonly Hashtable m_Table = new();

		public override TimeSpan CastDelayBase => TimeSpan.FromSeconds(0.25);

		public EvasionSpell(Mobile caster, Item scroll)
			: base(caster, scroll, BushidoSpellName.Evasion)
		{
		}
		...
		public override bool CheckCast()
		{
			...
			if (!Caster.CanBeginAction(typeof(EvasionSpell)))
			{
		...
		public override void OnCast()
		{
			if (CheckSequence())
			{
				...
				Caster.BeginAction(typeof(EvasionSpell));

				Timer.DelayCall(TimeSpan.FromSeconds(20.0), Caster.EndAction, typeof(EvasionSpell));
			}
		...
		public static TimeSpan GetEvadeDuration(Mobile m)
		{
			...
			{
				seconds += (m.Skills.Bushido.Value - 60.0) / 20.0;
			}
		...
		public static void BeginEvasion(Mobile m)
		{
			...
			m_Table[m] = t = new InternalTimer(m, GetEvadeDuration(m));
		...
		public static void EndEvasion(Mobile m)
		{
			...
			OnEffectEnd(m, BushidoSpellName.Evasion);
		}
