...
namespace Server.Items
{
	public class Block : WeaponAbility
	{
		...
		public override void OnHit(Mobile attacker, Mobile defender, int damage)
		{
			...
			var bonus = (int)(10.0 * ((Math.Max(attacker.Skills[SkillName.Bushido].Value, attacker.Skills[SkillName.Ninjitsu].Value) - 50.0) / 70.0 + 5));
		...
		{
			var info = m_Table[targ] as BlockInfo;
		...
		public static void BeginBlock(Mobile m, int bonus)
		{
			...
			var info = new BlockInfo(m, bonus) {
		...
		{
			var info = m_Table[m] as BlockInfo;
