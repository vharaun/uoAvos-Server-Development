...
namespace Server.Engines.Harvest
{
	public class Mining : HarvestSystem
	{
		...
		public static Mining System => m_System ??= new();
		...
		public override Type GetResourceType(Mobile from, IHarvestTool tool, HarvestDefinition def, Map map, Point3D loc, HarvestResource resource)
		{
			if (def == OreAndStone)
			{
				if (from is PlayerMobile pm && pm.StoneMining && pm.ToggleMiningStone && from.Skills.Mining.Base >= 100.0 && 0.1 > Utility.RandomDouble())
				{
		...
		public override bool CheckHarvest(Mobile from, IHarvestTool tool, HarvestDefinition def, object toHarvest)
		{
			...
			if (def == Sand && (from is not PlayerMobile pm || pm.Skills.Mining.Base < 100.0 || !pm.SandMining))
			{
		...
		public override HarvestVein MutateVein(Mobile from, IHarvestTool tool, HarvestDefinition def, HarvestBank bank, object toHarvest, HarvestVein vein)
		{
			...
			if (tool is GargoylesPickaxe && def == OreAndStone && 0.1 > Utility.RandomDouble())
			{
				...
				if (res == resource && res.Types.Length >= 3)
				{
					try
					{
						...
						var spawned = Utility.CreateInstance<BaseCreature>(res.Types[2], 25);
