	...
	public class DefBlacksmithy : CraftSystem
	{
		...
		public static void CheckAnvilAndForge(Mobile from, int range, out bool anvil, out bool forge)
		{
			...
			foreach (Item item in eable)
			{
				...
				var isAnvil = (type.IsDefined(typeofAnvil, false) || item.ItemID == 4015 || item.ItemID == 4016 || item.ItemID == 0x2DD5 || item.ItemID == 0x2DD6);
				var isForge = (type.IsDefined(typeofForge, false) || item.ItemID == 4017 || (item.ItemID >= 6522 && item.ItemID <= 6569) || item.ItemID == 0x2DD8);
