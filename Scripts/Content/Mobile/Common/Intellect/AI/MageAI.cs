...
namespace Server.Mobiles
{
	public class MageAI : BaseAI
	{
		...
		public virtual bool SmartAI => m_Mobile is BaseVendor || m_Mobile is Changeling;
		...
		{
			var bound = (m_Mobile.Skills[SkillName.Necromancy].Value >= 100) ? 5 : 3;
		...
		public virtual Spell ChooseSpell(Mobile c)
		{
			...
			if (!SmartAI)
			{
				...
				if (IsNecromancer)
				{
					var psDamage = ((m_Mobile.Skills[SkillName.SpiritSpeak].Value - c.Skills[SkillName.MagicResist].Value) / 10) + (c.Player ? 18 : 30);
		...
		private bool ProcessTarget()
		{
			...
			var isReveal = (targ is RevealSpell.InternalTarget);
			...
			var isInvisible = (targ is InvisibilitySpell.InternalTarget);
			...
			else if (isTeleport && toTarget != null)
			{
				...
				for (var i = 0; i < 10; ++i)
				{
					var randomPoint = new Point3D(m_Mobile.X - teleRange + Utility.Random(teleRange * 2 + 1), m_Mobile.Y - teleRange + Utility.Random(teleRange * 2 + 1), 0);
