...
namespace Server.Items
{
	...
	public class Corpse : Container, ICarvable
	{
		...
		private class InstancedItemInfo
		{
			...
			public bool Perpetual { get => m_Perpetual; set => m_Perpetual = value; }
		...
		private void AssignInstancedLoot()
		{
			...
			for (var i = 0; i < m_Stackables.Count; i++)
			{
				...
				if (item.Amount >= attackers.Count)
				{
					...
					{
						var splitItem = Mobile.LiftItemDupe(item, item.Amount - amountPerAttacker);  //LiftItemDupe automagically adds it as a child item to the corpse
