...
namespace Server.Items
{
	...
	public class OreCrate : BaseContainer
	{
		...
		public override void OnDoubleClick(Mobile from)
		{
			...
			{
				if (from.HasGump(typeof(MiningGump)))
                	from.CloseGump(typeof(MiningGump));

            	from.SendGump(new MiningGump(from, this));
			}	
			else
		...
		public class MiningGumpTarget : Target
        {
			OreCrate m_crate;

            public MiningGumpTarget(OreCrate crate): base(10, true, TargetFlags.None)
            {
				m_crate = crate;
            }
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
				...
				from.SendGump(new MiningGump(from, m_crate));

            }

        }

		public OreCrate(Serial serial) : base(serial)
		{
		}
