	...
	public class MyRunUO : Timer
	{
		...
		public void CollectMobiles(DateTime endTime)
		{
			if (Config.LoadDataInFile)
			{
				...
				for (var i = m_Index; i < m_Collecting.Count; ++i)
				{
					var acct = m_Collecting[i];

					for (var j = 0; j < acct.Length; ++j)
					{
						var mob = acct[j];
		...
		public void InsertMobile(Mobile mob)
		{
			...
			var notoTitle = SafeString(Titles.ComputeTitle(null, mob));
			var female = (mob.Female ? "1" : "0");

			var pubBool = (mob is PlayerMobile) && (((PlayerMobile)mob).PublicMyRunUO);

			var pubString = (pubBool ? "1" : "0");

			var guildId = (mob.Guild == null ? "NULL" : mob.Guild.Id.ToString());
		...
		public void InsertSkills(Mobile mob)
		{
			...
			var serial = mob.Serial.Value.ToString();
		...
		public void DumpMobiles(DateTime endTime)
		{
			...
			for (var i = m_Index; i < m_List.Count; ++i)
			{
		...
		{
			var wars = guild.Enemies;

			var ourId = guild.Id.ToString();

			for (var i = 0; i < wars.Count; ++i)
			{
				var them = wars[i];
				var theirId = them.Id.ToString();
