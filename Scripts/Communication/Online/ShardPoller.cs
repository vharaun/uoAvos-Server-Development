...
namespace Server.Misc
{
	public class ShardPoller : Item
	{
		...
		public TimeSpan TimeRemaining
		{
			get
			{
				...
				try
				{
					var ts = (m_StartTime + m_Duration) - DateTime.UtcNow;
		...
		{
			var index = Array.IndexOf(m_Options, option);
			...
			for (var i = index; i < m_Options.Length; ++i)
			{
				m_Options[i] = old[i + 1];
			}
		}
		...
		public void SendQueuedPoll_Callback(object state)
		{
			...
			var queue = (Queue<ShardPoller>)states[1];
	...
	public class ShardPollOption
	{
		...
		public string Title { get => m_Title; set { m_Title = value; m_LineBreaks = GetBreaks(m_Title); } }
		...
		public bool HasAlreadyVoted(NetState ns)
		{
			...
			for (var i = 0; i < m_Voters.Length; ++i)
			{
