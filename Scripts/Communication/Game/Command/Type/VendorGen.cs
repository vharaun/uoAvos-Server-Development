	...
	public class VendorGenerator
	{
		...
		private static void Process(Map map, Rectangle2D[] regions)
		{
			...
			for (var i = 0; i < m_ShopList.Count; ++i)
			{
				...
				for (var j = 0; !hasSpawner && j < si.m_Floor.Count; ++j)
				{
					var fp = (Point2D)si.m_Floor[j];
				...
				for (var j = 0; j < names.Count; ++j)
				{
					...
					for (var k = 0; k < si.m_Floor.Count; ++k)
					{
						var fp = (Point2D)si.m_Floor[k];
		...
		private static ShopFlags ProcessDisplayedItem(int itemID)
		{
			...
			var res = ShopFlags.None;

			var id = TileData.ItemTable[itemID];
			var flags = id.Flags;
		...
		private static void ProcessDisplayCase(Map map, StaticTile[] tiles, int x, int y)
		{
			...
			if (flags != ShopFlags.None)
			{
				...
				var si = (ShopInfo)m_ShopTable[p];

				if (si == null)
				{
					var floor = new ArrayList();
