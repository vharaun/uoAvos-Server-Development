...
namespace Server
{
	public class Statics
	{
		...
		private static Point3D NullP3D = new Point3D(Int32.MinValue, Int32.MinValue, Int32.MinValue);
			...
			if (start3d == NullP3D && end3d == NullP3D)
			{
				...
				foreach (var item in World.Items.Values)
				{
					...
					if (item is Static || item is BaseFloor || item is BaseWall)
					{
						...
						var table = (Hashtable)mapTable[itemMap];
			...
			foreach (DictionaryEntry de in mapTable)
			{
				...
				using (var idxStream = OpenWrite(matrix.IndexStream))
				{
					using (var mulStream = OpenWrite(matrix.DataStream))
					{
						...
						{
							int oldTileCount;
							var oldTiles = ReadStaticBlock(idxReader, mulStream, state.m_X, state.m_Y, matrix.BlockWidth, matrix.BlockHeight, out oldTileCount);
			...
			using (var idxStream = OpenWrite(matrix.IndexStream))
			{
				using (var mulStream = OpenWrite(matrix.DataStream))
				{
					...
					for (var x = xStartBlock; x <= xEndBlock; ++x)
					{
						...
						{
							int oldTileCount;
							var oldTiles = ReadStaticBlock(idxReader, mulStream, x, y, matrix.BlockWidth, matrix.BlockHeight, out oldTileCount);
