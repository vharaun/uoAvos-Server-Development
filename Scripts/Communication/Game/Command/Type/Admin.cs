	...
	public class AdminGump : Gump
	{
				...
				case AdminGumpPage.Clients:
					{
						...
						{
							var ns = m_List[index] as NetState;
				...
				case AdminGumpPage.Accounts_Shared:
					{
						...
						for (int i = 0, index = (listPage * 12); i < 12 && index >= 0 && index < m_List.Count; ++i, ++index)
						{
							var de = (DictionaryEntry)m_List[index];

							var ipAddr = (IPAddress)de.Key;
							var accts = (ArrayList)de.Value;
		...
		private class SharedAccountComparer : IComparer
		{
			...
			public int Compare(object x, object y)
			{
				...
				var aList = (ArrayList)a.Value;
				var bList = (ArrayList)b.Value;
		...
		public static void ResendGump_Callback(Mobile from, object state)
		{
			...
			var list = (ArrayList)states[0];
			var rads = (ArrayList)states[1];
			var page = (int)states[2];
		...
		public static void Marked_Callback(Mobile from, bool okay, object state)
		{
			...
			var states = (object[])state;
			var ban = (bool)states[0];
			var list = (ArrayList)states[1];
			var rads = (ArrayList)states[2];
			var page = (int)states[3];
		...
		public static void Firewall_Callback(Mobile from, bool okay, object state)
		{
			...
			var a = (Account)states[0];
			var toFirewall = states[1];
							...
							case 211:
								{
									...
									var text = (relay == null ? null : relay.Text.Trim());
							...
							case 1:
								{
									...
									var match = (matchEntry == null ? null : matchEntry.Text.Trim().ToLower());
									string notice = null;
									...
									{
										var instances = NetState.Instances;
				...
				case 5:
					{
						switch (index)
						{
							...
							case 6:
								{
									var unEntry = info.GetTextEntry(0);
									var pwEntry = info.GetTextEntry(1);

									var un = (unEntry == null ? null : unEntry.Text.Trim());
									var pw = (pwEntry == null ? null : pwEntry.Text.Trim());
							...
							case 12:
								{
									...
									var password = (passwordEntry == null ? null : passwordEntry.Text.Trim());
									var confirm = (confirmEntry == null ? null : confirmEntry.Text.Trim());
							...
							case 19: // add
								{
									...
									var ip = (entry == null ? null : entry.Text.Trim());
				...
				case 6:
					{
						switch (index)
						{
							case 0:
								{
									var matchEntry = info.GetTextEntry(0);
									var match = (matchEntry == null ? null : matchEntry.Text.Trim());
				...
				case 8:
					{
						if (m_List != null && index >= 0 && index < m_List.Count)
						{
							...
							{
								var list = new ArrayList(a.IPRestrictions);
				...
				case 9:
					{
						if (m_List != null && index >= 0 && index < m_List.Count)
						{
							if (m_PageType == AdminGumpPage.AccountDetails_Access_ClientIPs)
							{
								...
								var list = GetSharedAccounts((IPAddress)obj);
