...
namespace Server.Engines.Reports
{
	public abstract class BaseInfo : IComparable
	{
		...
		public string Display
		{
			get
			{
				...
				if (m_Account != null)
				{
					var acct = Accounts.GetAccount(m_Account);
		...
		{
			var cmp = obj as BaseInfo;

			var v = cmp.GetPageCount(cmp is StaffInfo ? PageResolution.Handled : PageResolution.None, DateTime.UtcNow - m_SortRange, DateTime.UtcNow)
	...
	public class PageInfo : PersistableObject
	{
		...
		public void UpdateResolver()
		{
			...
			var res = GetResolution(out resolvedBy, out timeResolved);
		...
		{
			for (var i = m_Responses.Count - 1; i >= 0; --i)
			{
				var resp = m_Responses[i];
				var res = ResFromResp(resp.Message);
		...
		public static string GetAccount(Mobile mob)
		{
			...
			var acct = mob.Account as Accounting.Account;
	...
	public class PageInfoCollection : System.Collections.CollectionBase
	{
		...
		{
			get => ((Server.Engines.Reports.PageInfo)(List[index]));
	...
	public class QueueStatusCollection : System.Collections.CollectionBase
	{
		...
		{
			get => ((Server.Engines.Reports.QueueStatus)(List[index]));
			set => List[index] = value;
		}
	...
	public class ResponseInfoCollection : System.Collections.CollectionBase
	{
		...
		{
			get => ((Server.Engines.Reports.ResponseInfo)(List[index]));
			set => List[index] = value;
		}
	...
	public class StaffHistory : PersistableObject
	{
		...
		public PageInfoCollection Pages { get => m_Pages; set => m_Pages = value; }
		public QueueStatusCollection QueueStats { get => m_QueueStats; set => m_QueueStats = value; }

		public Hashtable UserInfo { get => m_UserInfo; set => m_UserInfo = value; }
		public Hashtable StaffInfo { get => m_StaffInfo; set => m_StaffInfo = value; }
		...
		public StaffInfo GetStaffInfo(string account)
		{
			lock (RenderLock)
			{
				...
				var info = m_StaffInfo[account] as StaffInfo;
		...
		public UserInfo GetUserInfo(string account)
		{
			...
			var info = m_UserInfo[account] as UserInfo;
		...
		public void Save()
		{
			lock (SaveLock)
			{
				var path = Path.Combine(Core.BaseDirectory, "staffHistory.xml");
		...
		public override void SerializeChildren(PersistanceWriter op)
		{
			...
			for (var i = 0; i < m_QueueStats.Count; ++i)
			{
		...
		public override void DeserializeChildren(PersistanceReader ip)
		{
			var min = DateTime.UtcNow - TimeSpan.FromDays(8.0);
			...
			while (ip.HasChild)
			{
				...
				{
					var queueStatus = obj as QueueStatus;
		...
		{
			var staff = new StaffInfo[m_StaffInfo.Count];
			var index = 0;
		...
		private BarGraph GraphQueueStatus()
		{
			...
			var min = max - TimeSpan.FromDays(7.0);

			for (var i = 0; i < m_QueueStats.Count; ++i)
			{
				var ts = m_QueueStats[i].TimeStamp;
			...
			var barGraph = new BarGraph("Average pages in queue", "graph_pagequeue_avg", 10, "Time", "Pages", BarGraphRenderMode.Lines) {
				FontSize = 6
			};
		...
		private BarGraph GraphHourlyPages(PageInfoCollection pages, PageResolution res, string title, string fname)
		{
			...
			var dates = new DateTime[24];

			var max = DateTime.UtcNow;
			var min = max - TimeSpan.FromDays(7.0);

			var sentStamp = (res == PageResolution.None);
			...
			for (var i = 0; i < pages.Count; ++i)
			{
				...
				var ts = (sentStamp ? pages[i].TimeSent : pages[i].TimeResolved);
			...
			var barGraph = new BarGraph(title, fname, 10, "Time", "Pages", BarGraphRenderMode.Lines) {
				FontSize = 6
			};
		...
		private Report ReportTotalPages(StaffInfo[] staff, TimeSpan ts, string title)
		{
			...
			var report = new Report(title + " Staff Report", "400");
		...
		private PieChart[] ChartTotalPages(StaffInfo[] staff, TimeSpan ts, string title, string fname)
		{
			...
			var staffChart = new PieChart(title + " Staff Chart", fname + "_staff", true);

			var other = 0;

			for (var i = 0; i < staff.Length; ++i)
			{
				var count = GetPageCount(staff[i], min, max);
			...
			var resChart = new PieChart(title + " Resolutions", fname + "_resol", true);

			var countTotal = GetPageCount(m_Pages, PageResolution.None, min, max);
			var countHandled = GetPageCount(m_Pages, PageResolution.Handled, min, max);
			var countDeleted = GetPageCount(m_Pages, PageResolution.Deleted, min, max);
			var countCanceled = GetPageCount(m_Pages, PageResolution.Canceled, min, max);
			var countLogged = GetPageCount(m_Pages, PageResolution.Logged, min, max);
			var countUnres = countTotal - (countHandled + countDeleted + countCanceled + countLogged);
